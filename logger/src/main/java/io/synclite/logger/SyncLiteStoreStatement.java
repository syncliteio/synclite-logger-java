...
public class SyncLiteStoreStatement extends JDBC4Statement {
    ...
    protected boolean executeSingleSQL(String sql) throws SQLException {
        ...
        if (tokens[0].equalsIgnoreCase("INSERT") && tokens[1].equalsIgnoreCase("INTO")) {
            ...
            try {
                SyncLiteUtils.checkAndExecuteInternalStoreSql(sql);
    ...
    private final boolean executeDML(String sql) throws SQLException {
        StringBuilder sqlToLog = new StringBuilder();
        boolean result = stmtExecute(sql, null, sqlToLog);
        log(sqlToLog.toString());
        processCommit();
        return result;
    }
